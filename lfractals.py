...
def drawcircs(x1, y1, size, radius, depth):
    global colordex
    if depth:
        x2 = x1 + 2 * int(math.cos(math.radians(branch + depth)) * (size + depth))
        y2 = y1 + 2 * int(math.sin(math.radians(branch + depth)) * (size + depth))
        pygame.draw.circle(screen, colordex[depth], (x2, y2), size - depth, 2)
        drawcircs(x2, y2, size, angle - branch + lean, depth - 1)

def drawshapes():
    screen.fill((BLACK))
    drawTree(xo, yo, size, angle, depth)
    #drawcircs(xo, yo, size, angle, depth)
